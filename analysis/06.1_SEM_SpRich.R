...
m3_Abund_exp <- glmer.nb(Abund_D_E_exper ~  Grazing_intensity  +  Grazer_diversity +  
                        ...
                        data=SEM.dat, control=glmerControl(optimizer="bobyqa")) 

check_convergence(m3_Abund_exp)

check_overdispersion(m3_Abund_exp)

m3b_Abund_exp <- glmmPQL(Abund_D_E_exper ~  Grazing_intensity  +  Grazer_diversity +  
                           Excrements_cover + Manuring_freq, random = ~ 1|Farm,
                         data=SEM.dat, family="quasipoisson") 



Anova(m3b_Abund_exp)
summary(m3b_Abund_exp)
